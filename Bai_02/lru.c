...
int main()
{
  ...
  // run lru algorithm and display step by step
  printf("\nSTT Page\tFrames\n");
  for (int i = 0; i < number_page; ++i)
  {
    printf("%2d   %3d\t", i, arr_pages[i]);
    int pos = lru_paging(number_frame, arr_frames,
                         arr_pages, i);
    if (pos != UNDEFINE)
...
void init_frames(int **arr_frames, int *number_frame)
{
  ...
  *arr_frames = (int *)malloc(*number_frame * sizeof(int));
  arr_time_pages = (int *)malloc(*number_frame *
                                 sizeof(int));
               ...
               int *arr_pages, int index_page)
{
  ...
  for (int i = 0; i < number_frame; i++)
  {
    ...
    if (arr_time_pages[i] < arr_time_pages[index_min_time])
      index_min_time = i;
  }
