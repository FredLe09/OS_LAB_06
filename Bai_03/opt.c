...
int opt_paging(int number_frame, int *arr_frames, int number_page, int *arr_pages, int current_index)
{
    int pos = UNDEFINE;
    ...
    for (int i = 0; i < number_frame; i++)
    {
        ...
        if (arr_frames[i] == arr_pages[current_index])
            return UNDEFINE;
